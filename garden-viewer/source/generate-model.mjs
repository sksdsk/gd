...
const scene=new T.Scene();scene.name='Wuhan_Existing_Site_R0';scene.userData={status:config.status,units:'metres',north:'UNKNOWN',dimensionStatus:config.dimensionStatus,levelStatus:config.groundLevels};
...
for(const [side,x]of [['planter',A-.025],['bare',B+.025]]){
 ...
 const dirs=side==='planter'?[6.6,8.8]:[4.1,7.5];
...
const brickRegion={x0:.15,x1:A-.1,z0:.15,z1:3.65};
box(pave,'Red_Brick_Mortar_Base',A/2,.02,1.9,A-.25,.04,3.5,'brick');
const bricks=group('Red_Brick_Joints_Indicative','paving',pave);
for(let row=0,z=.15;z<3.62;z+=.13,row++)for(let x=.15+(row%2)*.12;x<A-.12;x+=.245){const w=Math.min(.232,A-.1-x);if(w>.035)box(bricks,'Brick',x+w/2,.046,z+.058,w,.026,.116,(row%9===0&&Math.round(x*10)%5===0)?'brickBlue':'brick');}
merge(bricks);
box(pave,'Grey_Access_Route_Approximate',A/2,.028,6.65,A-.08,.055,1.02,'slab');
box(pave,'Facade_Access_Approximate',A-.38,.029,7.25,.7,.056,3.4,'slab');
const pass=group('Zone_3_Steps_And_Gravel','passage');box(pass,'Gravel_Path',L/2,.015,.94,B-A+3.6,.035,.88,'gravel');
for(let x=A-1.65;x<B+1.6;x+=.89)box(pass,'Existing_Square_Slab',x,.045,.94,d.pathSlabLength,.065,d.pathSlabWidth,'slab');
box(pass,'Narrow_Brick_Edge_Outside',L/2,.04,.47,B-A+3.6,.055,.075,'brick');box(pass,'Narrow_Brick_Edge_Inside',L/2,.04,1.41,B-A+3.6,.055,.075,'brick');merge(pass);
const bedStart=.35;
for(let i=0;i<2;i++){
...
const f=group('Boundary_Fences_Approximate','boundary');fence(f,'Side',0,0,L,0);fence(f,'Planter_Outer_1',0,0,0,6.06);fence(f,'Planter_Outer_2',0,7.3,0,W);fence(f,'Bare_Outer_1',L,0,L,5.7);fence(f,'Bare_Outer_2',L,6.9,L,W);fence(f,'Bare_Divider',B,W,L,W);fence(f,'Planter_Divider',0,W,A,W);merge(f);
const gate=group('Gates_And_Low_Walls_Approximate','boundary');box(gate,'Gate_Pier_Left',0,.73,6.06,.22,1.46,.19,'wall');box(gate,'Gate_Pier_Right',0,.73,7.26,.22,1.46,.19,'wall');box(gate,'Low_Wall',0,.43,4.69,.18,.86,2.55,'wall');fence(gate,'Gate_Closed_Reference',0,6.18,0,7.13);box(gate,'Gate_Leaf_Base',0,.38,6.66,.045,.7,1,'metal');merge(gate);
